...
export const JobView = ({ data }) => {
   ...
   useEffect(() => {
      ...
      if (data.id && Authentication.isUserAuthenticated()) {
         isSavedJob(data.id, Authentication.getCurrentUser().id).then((res) => {
      ...
      <div className="space-y-3 rounded-xl w-full h-xl bg-white p-5 hover:bg-gray-50">
         ...
         <div className="items-stretch">
            <div className="flex flex-row space-x-3 items-center">
               {/* LogoCompany */}
               <img onClick={handlerClickViewJobDetail} className="rounded-md w-10 h-10" src={companyLogo} alt={logoJobFinder} />
