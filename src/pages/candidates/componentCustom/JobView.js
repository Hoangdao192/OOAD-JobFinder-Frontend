...
export const JobView = ({ data }) => {
  ...
  return (
    <div className="space-y-2 rounded-xl w-full shadow-md bg-white p-4">
      <div className="Header space-y-2">
        <div className="flex flex-row space-x-2 items-top">
          ...
          <div>
            <label className="text-lg font-medium">{data.jobTitle}</label>

            <div className="flex flex-row space-x-2 items-center text-xs">
              <label className="line-clamp-1 text-gray-500">
                {data.workingForm}
              </label>
              <img alt="." className="w-1 h-1" src={DotDivide} />
              <label className="line-clamp-1 text-gray-500">
                {data.jobAddress.province}
              </label>
              <img alt="." className="w-1 h-1" src={DotDivide} />
              <label className="line-clamp-1 text-gray-500">
                {data.numberOfHiring} Ứng viên
              </label>
            </div>
          </div>
      ...
      <p className="text-base line-clamp-3">{data.jobDescription}</p>

      <div className="flex flex-row gap-5">
        <div className="bg-slate-100 text-xs p-1 rounded-md line-clamp-1">
          {data.major}
        </div>
        <div className="bg-slate-100 text-xs p-1 rounded-md line-clamp-1">
          {data.salary}
